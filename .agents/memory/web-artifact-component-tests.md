...
**How to apply:** Keep component tests on `node:test` and `react-dom/server` when static state rendering is sufficient, and pass the web package tsconfig to `tsx`. For jsdom interaction tests, install the DOM and its event constructors on `globalThis` before dynamically importing `react-dom/client`; importing ReactDOM first makes it choose its legacy input-event fallback.